...
export default function PdfToPptPage() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      alert('PDF to PowerPoint conversion requires server-side processing.')
    } finally {
          ...
          <div className="flex gap-8">
            <div className="flex-1 max-w-4xl mx-auto">
              <div className="text-center mb-8">
                ...
                <h1 className="text-3xl md:text-4xl font-bold text-foreground">PDF to PowerPoint</h1>
                <p className="mt-4 text-muted-foreground text-lg max-w-xl mx-auto">
                  Convert PDF files to editable presentations.
                </p>
              </div>
                ...
                {files.length > 0 && (
                  <div className="mt-6">
                    <Button size="lg" className="w-full text-base" onClick={handleConvert} disabled={isProcessing}>
                      {isProcessing ? (<><Loader2 className="mr-2 h-5 w-5 animate-spin" />Converting...</>) : (<><Download className="mr-2 h-5 w-5" />Convert to PPT</>)}
                    </Button>
                  </div>
                )}
              </div>
