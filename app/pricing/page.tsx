...
import { PRODUCTS, getMonthlyPrice, type Product } from '@/lib/products'
...
export default function PricingPage() {
  ...
  const premiumPlan = isYearly 
    ? PRODUCTS.find(p => p.id === 'premium-yearly')!
    : PRODUCTS.find(p => p.id === 'premium-monthly')!
  const businessPlan = isYearly
    ? PRODUCTS.find(p => p.id === 'business-yearly')!
    : PRODUCTS.find(p => p.id === 'business-monthly')!
      ...
      <main className="container mx-auto px-4 py-16">
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          ...
          <Card className="relative border-2 border-primary shadow-xl scale-105">
            ...
            <CardContent className="text-center">
              <div className="mb-6">
                <span className="text-5xl font-bold">
                  ${(getMonthlyPrice(premiumPlan) / 100).toFixed(0)}
                </span>
                <span className="text-muted-foreground">/month</span>
                {isYearly && (
                  <p className="text-sm text-muted-foreground mt-1">
                    Billed ${(premiumPlan.priceInCents / 100).toFixed(0)} yearly
                  </p>
                )}
              </div>
              <ul className="space-y-3 text-left">
            ...
            <CardFooter>
              <Link href={`/checkout?plan=${premiumPlan.id}`} className="w-full">
          ...
          <Card className="relative border-2 hover:border-primary/50 transition-colors">
            ...
            <CardContent className="text-center">
              <div className="mb-6">
                <span className="text-5xl font-bold">
                  ${(getMonthlyPrice(businessPlan) / 100).toFixed(0)}
                </span>
                <span className="text-muted-foreground">/month</span>
                {isYearly && (
                  <p className="text-sm text-muted-foreground mt-1">
                    Billed ${(businessPlan.priceInCents / 100).toFixed(0)} yearly
                  </p>
                )}
              </div>
              <ul className="space-y-3 text-left">
            ...
            <CardFooter>
              <Link href={`/checkout?plan=${businessPlan.id}`} className="w-full">
                <Button variant="outline" className="w-full" size="lg">
                  Subscribe Now
                </Button>
              </Link>
        ...
        <div className="mt-20 text-center bg-primary/5 rounded-2xl p-12">
          ...
          <p className="text-muted-foreground mb-8 max-w-xl mx-auto">
            Join millions of users who trust OrbixDocs for their PDF needs. Start with our free plan and upgrade when you need more.
          </p>
